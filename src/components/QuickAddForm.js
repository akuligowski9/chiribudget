...
import { ALL_CATEGORIES, CURRENCIES, PAYERS, USD_THRESHOLD, FX_USD_TO_PEN } from "@/lib/categories";
...
export default function QuickAddForm() {
  ...
  const [txn_date, setTxnDate] = useState(() => new Date().toISOString().slice(0, 10));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: p } = await supabase.from("profiles").select("household_id, default_currency").eq("user_id", user.id).maybeSingle();
  ...
  async function onSubmit(e) {
    ...
    if (!amount || Number(amount) <= 0) {
      setToast({ id: toastId(), type: "error", title: "Invalid amount", message: "Enter a positive number." });
      return;
    }

    if (!demoMode && !householdId) {
      setToast({ id: toastId(), type: "error", title: "Setup required", message: "Create or join a household first." });
      return;
    }
    ...
    const fingerprint = demoMode
      ? `demo_${Date.now()}`
      : computeFingerprint({ household_id: householdId, currency, txn_date, amount: numericAmount, description });
    ...
    const row = {
      ...
      flag_reason: (Math.abs(numericAmount) > thr)
        ? (numericAmount < 0 ? "over_threshold_expense" : "over_threshold_income")
        : null,
      source: "manual",
      fingerprint,
    ...
    <div>
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 12 }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <label>
            Date&nbsp;
            <input type="date" value={txn_date} onChange={(e) => setTxnDate(e.target.value)} />
          </label>
        ...
        <label>
          ...
          <div style={{ fontSize: 12, opacity: 0.75, marginTop: 4 }}>
            Threshold flag: {currency} {thr} (flagged items reviewed on Dashboard)
          </div>
        </label>
        ...
        <label>
          Payer
          <select value={payer} onChange={(e) => setPayer(e.target.value)} style={{ width: "100%", padding: 10 }}>
        ...
        <label>
          ...
          <input value={description} onChange={(e) => setDescription(e.target.value)} style={{ width: "100%", padding: 10 }} />
        </label>

        <button type="submit" style={{ padding: 12, fontWeight: 700 }}>Save</button>
      </form>
