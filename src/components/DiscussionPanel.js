...
export default function DiscussionPanel({ month, currency }) {
  ...
  async function markDiscussed() {
    ...
    if (unresolvedFlagged > 0) {
      setToast({
        ...
        type: 'error',
        title: 'Blocked',
        message: 'Resolve flagged explanations before marking Discussed.',
      });
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ opacity: 0.8 }}>
        ...
        {statusRow?.discussed_at ? (
          <>
            {' '}
            • Discussed on{' '}
            <b>{new Date(statusRow.discussed_at).toLocaleString()}</b>
          </>
        ) : null}
      </div>
      ...
      >
        <button onClick={saveNotes} style={{ padding: '10px 12px' }}>
          Save Notes
        </button>
        <button
