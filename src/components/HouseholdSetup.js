...
export default function HouseholdSetup({ onReady }) {
  ...
  return (
    <section style={{ marginTop: 14, border: "1px solid #e5e7eb", borderRadius: 12, padding: 16 }}>
      ...
      <p style={{ opacity: 0.8 }}>
        Create a household or join one using a join code. This makes the repo fork-friendly.
      </p>

      <div style={{ display: "grid", gap: 12 }}>
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 700 }}>Create Household</div>
          <input value={householdName} onChange={(e) => setHouseholdName(e.target.value)} style={{ width: "100%", padding: 10, marginTop: 8 }} />
          <button onClick={createHousehold} style={{ padding: "10px 12px", marginTop: 8 }}>Create</button>
        </div>

        <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 700 }}>Join Household</div>
          <input value={joinCode} onChange={(e) => setJoinCode(e.target.value)} placeholder="Join code (e.g. a1b2c3d4e5f6)" style={{ width: "100%", padding: 10, marginTop: 8 }} />
          <button onClick={joinHousehold} style={{ padding: "10px 12px", marginTop: 8 }}>Join</button>
        </div>
