...
import { EXPENSE_CATEGORIES, INCOME_CATEGORIES, PAYERS } from "@/lib/categories";
...
export default function DashboardCharts({ month, currency }) {
  ...
  const incomeByCat = useMemo(() => {
    ...
    for (const r of incomeRows) m[r.category] = (m[r.category] || 0) + Number(r.amount);
    return m;
  }, [rows]);

  const expenseByCat = useMemo(() => {
    const m = Object.fromEntries(EXPENSE_CATEGORIES.map(c => [c, 0]));
    for (const r of expenseRows) m[r.category] = (m[r.category] || 0) + Math.abs(Number(r.amount));
    return m;
  }, [rows]);
  ...
  return (
    <section style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16 }}>
      ...
      {!demoMode && !householdId && <p style={{ opacity: 0.8 }}>Log in and set up a household to see live data.</p>}

      <div style={{ display: "grid", gap: 12 }}>
        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
          <div><b>Total Income:</b> {currency} {totalIncome.toFixed(2)}</div>
          <div><b>Total Expenses:</b> {currency} {totalExpenses.toFixed(2)}</div>
          <div><b>Net:</b> {currency} {net.toFixed(2)}</div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 12 }}>
            ...
            {INCOME_CATEGORIES.map(c => (
              <div key={c} style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                <span>{c}</span><span>{currency} {Number(incomeByCat[c] || 0).toFixed(2)}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 12 }}>
            ...
            {EXPENSE_CATEGORIES.map(c => (
              <div key={c} style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                <span>{c}</span><span>{currency} {Number(expenseByCat[c] || 0).toFixed(2)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ border: "1px solid #eee", borderRadius: 12, padding: 12 }}>
          ...
          {PAYERS.map(p => (
            <div key={p} style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
              <span>{p}</span><span>{currency} {Number(netByPayer[p] || 0).toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
