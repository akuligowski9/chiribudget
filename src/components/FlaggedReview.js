...
export default function FlaggedReview({ month, currency }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (getDemoMode()) {
        const tx = getDemoTransactions({ month, currency }).filter(t => t.is_flagged);
        setRows(tx);
        return;
      }
      ...
      const { data: tx, error } = await supabase
        .from("transactions")
        .select("id,txn_date,description,amount,currency,category,payer,is_flagged,explanation")
  ...
  async function saveExplanation(id, explanation) {
    ...
    try {
      ...
      setToast({ id: toastId(), type: "error", title: "Save failed", message: e.message });
    }
  }

  const unresolved = rows.filter(r => !(r.explanation && r.explanation.trim().length > 0));
  ...
  return (
    <section style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 16 }}>
      ...
      {rows.length === 0 ? (
        <p style={{ opacity: 0.8 }}>No flagged transactions for this month/currency.</p>
      ) : (
        <div style={{ display: "grid", gap: 10 }}>
          {rows.map(r => (
            <div key={r.id || `${r.txn_date}-${r.amount}-${r.description}`} style={{ border: "1px solid #eee", borderRadius: 12, padding: 12 }}>
